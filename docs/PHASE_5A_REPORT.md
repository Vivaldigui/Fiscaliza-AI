...
134 testes novos/atualizados, todos com fixtures sintéticas, nenhum dependente de API externa:

- **`packages/ai` (38, `node:test`)**: `embeddings.test.ts` (hash, batching, versionamento), `fake-embedding.provider.test.ts`, `openai-embedding.provider.test.ts` (stub do client, sem rede), fábricas e schemas existentes atualizados.
- **`apps/worker` unit (31, Jest)**: `embeddings-indexer.spec.ts` (desabilitado = sem leitura; tentativa ausente/antiga; `NEEDS_REVIEW` fail-closed; idempotência por hash; grava só pendentes e registra `AIUsage`), `conversation-answer-pipeline.spec.ts` (injeção adversária não altera a verdade do banco; injeção pura retorna `null`; deny-by-default sem proposição e sem chamada ao LLM; `CHAT_ENABLED=false` não lê a mensagem e marca `FAILED`), `config.spec.ts` (fake aceito em dev; fake rejeitado em produção para LLM e embeddings; chave obrigatória para provider real; `CHAT_ENABLED` exige IA + embeddings) e demais suítes atualizadas para o novo `WorkerConfig`.
- **`apps/worker` integração (18, Jest + PostgreSQL/pgvector real)**: `retrieval.integration-spec.ts` (5) — **não retorna chunk fora do escopo autorizado mesmo quando ele é o mais similar (mandatório)**; allowlist só com documentos vinculados à proposição (inclui respostas oficiais, exclui demais); filtra para a tentativa corrente; ignora versão de embeddings antiga; allowlist vazio → nenhuma busca; `conversation-pipeline.integration-spec.ts` (6) — pipeline completo: estruturado sem LLM, RAG com fonte validada, fonte inventada → insuficiência, sem proposição → determinística sem LLM, `CHAT_ENABLED=false` → `FAILED`, duplicação ignorada; `ai-pipeline.integration-spec.ts` (7, Fase 4) sem regressão.
...
| testes unitários (`pnpm test`, monorepo)              | 116 passaram, 0 falharam                                                |
| testes de integração `@fiscaliza/worker` (PostgreSQL) | 18 passaram (5A retrieval + 5A conversa + 4 IA, sem regressão)          |
